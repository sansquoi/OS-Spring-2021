...
{	
  ...
  else if (p->uid ==ip->uid)	
  {
   ...
   else if((!strncmp(p->name,"exec", 4) | !strncmp(p->name,"ls",2)) & ((ip->perms & mode) & (mode & S_IXOTH)))	
   {
  	return true;	
	}
  else	
...
ichmod(struct inode *ip,  ushort mode, struct proc *p)
{
  ...
  if(p->uid==0)
	{	
		//Set perms to current mode
		ip->perms= mode;
		return 0;
	}
  else if(p->uid == ip->uid)  
	{
		//inode and proc uid match
		//Set perms to current mode
		ip->perms = mode;
		return 0;
	}
